...
def byte_corrupt(img, seed, n=4000):
    ...
    rng = np.random.default_rng(seed)
    for _ in range(n):
        i = int(rng.integers(0, len(b))); b[i] = int(rng.integers(0, 256))
...
def drag(img, seed, rows_frac=0.5, decay=0.82, min_len=30, max_len=200):
    ...
    for y in ys:
        x = int(rng.integers(0, w-10)); L = int(rng.integers(min_len, max_len))
...
def row_displace(img, seed, n=80, max_shift=140, big_prob=0.5):
    ...
    for y0, y1 in zip(bounds[:-1], bounds[1:]):
        sh = int(rng.integers(-max_shift, max_shift)) if rng.random() < big_prob else int(rng.integers(-8, 8))
        if sh: out[y0:y1] = np.roll(out[y0:y1], sh, axis=1)
    return out
