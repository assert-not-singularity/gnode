...
def band_displace(arr, seed=0, n_bands=48, max_shift=70,
                  p_big=0.45, small_shift=6):
    ...
    cuts = np.sort(rng.choice(np.arange(1, h), size=min(n_bands - 1, h - 2),
                              replace=False))
...
def band_displace_sine(arr, seed=0, n_bands=24, max_shift=60,
                       noise=0.18, power=1.3, width_var=0.6):
...
def cmy_split(arr, offset=7, angle=0):
    ...
    def shifted(deg):
        a = np.radians(deg)
        return np.roll(L, (int(round(offset * np.sin(a))),
                           int(round(offset * np.cos(a)))), axis=(0, 1))
